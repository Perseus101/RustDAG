use dag::milestone::{
    pending::{MilestoneError, _MilestoneErrorTag},
    Milestone,
};
...
use super::state::{PendingMilestoneState, PendingState, SigningState, StateUpdate};
...
impl PendingMilestone {
    ...
    pub fn new(transaction: Transaction, previous_milestone: Milestone) -> Self {
        ...
        {
            let transaction_chain = vec![(transaction.get_hash(), transaction.get_contract())];
            PendingMilestone::Signing(SigningState::new(
                transaction,
                milestone_hash,
                transaction_chain,
            ))
    ...
    pub fn next(&mut self, event: StateUpdate) -> Result<(), MilestoneError> {
        ...
        replace_with_or_abort(self, |_self| {
            let out = match _self {
                PendingMilestone::Pending(pending) => pending.next(&event),
                PendingMilestone::Signing(signing) => signing.next(&event),
                PendingMilestone::Approved(_) => match event {
...
mod tests {
    ...
    use dag::transaction::{data::TransactionData, header::TransactionHeader, Transaction};
    ...
    fn raw_update(pending: &PendingMilestone, update: StateUpdate) -> Result<(), MilestoneError> {
        let mut _pending = pending.clone();
        let res = _pending.next(update);
        if let Err(_) = &res {
            match pending {
                PendingMilestone::Pending(_) => match _pending {
                    PendingMilestone::Pending(_) => {}
                    _ => panic!("Unexpected state transition"),
                },
                PendingMilestone::Signing(_) => match _pending {
                    PendingMilestone::Signing(_) => {}
                    _ => panic!("Unexpected state transition"),
                },
                PendingMilestone::Approved(_) => match _pending {
    ...
    fn test_pending_milestone_state() {
        ...
        match raw_update(
            &pending,
            StateUpdate::Sign(MilestoneSignature::new(hash, 0, 0)),
        ) {
    ...
    fn test_signing_milestone_state() {
        ...
        match update(
            &pending,
            StateUpdate::Sign(MilestoneSignature::new(hash, 1, 0)),
        ) {
    ...
    fn test_approved_state() {
        ...
        assert!(pending
            ...
            .is_ok());
        assert!(pending
            .next(StateUpdate::Sign(MilestoneSignature::new(hash, 2, 0)))
            .is_ok());
