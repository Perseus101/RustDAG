...
use dag::{
    milestone::{
        pending::{MilestoneSignature, PendingMilestone, _MilestoneErrorTag},
        Milestone,
    },
...
impl SigningState {
    pub fn new(transaction: Transaction, previous_milestone: u64, chain: Vec<(u64, u64)>) -> Self {
        let mut signatures: HashMap<u64, bool> = HashMap::with_capacity(chain.len());
        let transaction_chain: Vec<u64> = chain
            .into_iter()
            .map(|(hash, contract)| {
...
impl PendingMilestoneState for SigningState {
    fn next(mut self, event: &StateUpdate) -> Result<PendingMilestone, _MilestoneErrorTag> {
        match event {
            StateUpdate::Chain(_) => Err(_MilestoneErrorTag::StaleChain(
                PendingMilestone::Signing(self),
            )),
            StateUpdate::Sign(signature) => {
                self.sign(signature);
                if self.signatures.values().all(|value| *value) {
                    Ok(PendingMilestone::Approved(Milestone::new(
...
mod tests {
    ...
    fn test_signing_state() {
        ...
        let pending = SigningState::new(
            transaction.clone(),
            hash,
            vec![
                (
                    trunk_transaction.get_hash(),
                    trunk_transaction.get_contract(),
                ),
        ...
        match pending
            .clone()
            .next(&StateUpdate::Chain(unrelated_transaction.clone()))
        {
        ...
        {
            Ok(PendingMilestone::Signing(pending)) => {
